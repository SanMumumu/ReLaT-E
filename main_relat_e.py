...
def run(rank, cfg, ckpt_path=None):
    ...
    try:
        ...
        if rgb_vae.latent_size != depth_vae.latent_size or rgb_vae.latent_frames != depth_vae.latent_frames:
            raise ValueError("RGB and depth VAE latent grids must match for ReLaT-E.")
        latent_size = int(rgb_vae.latent_size)
        latent_frames = int(rgb_vae.latent_frames)
        cfg.generator.mot.input_size = latent_size
