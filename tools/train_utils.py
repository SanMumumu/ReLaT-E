...
def run_evaluation(val_loader, ema, vae_cond_model, vae_pred_model, args, device, it, logger, log_):
    ...
    for x_val, _ in val_loader:
        ...
        c_feat_val = unwrapped_vae_cond.extract(x_cond_val).detach()
        
        # ==================== A. VAE Reconstruction ====================
        if hasattr(unwrapped_vae_cond, 'decode_from_sample'):
            pred_cond_val = unwrapped_vae_cond.decode_from_sample(c_feat_val)
        else:
            out_cond_val = unwrapped_vae_cond.decode(c_feat_val)
